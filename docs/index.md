...
> **Note**: This provider requires AutoMQ Control Plane 8.0 or later. `K8S` scheduling with `instance_types`, `kubernetes_load_balancer_subnets`, and `schedule_spec` requires Control Plane 8.3.6 or later. Cloud-specific minimum versions are listed below.

> **Migration from v0.2.3 or earlier**: The current provider schema contains breaking changes. After upgrading the target Control Plane, update the provider, align the Terraform configuration with the current schema, and re-import managed resources.
...
### Example Usage for a GCP BYOC environment

Requires AutoMQ Control Plane 8.3.8 or later and a GKE Standard cluster.
