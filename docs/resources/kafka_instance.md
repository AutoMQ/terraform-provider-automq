...
resource "automq_kafka_instance" "example" {
  ...
  compute_specs = {
    ...
    # security_groups = ["sg-example123"] # Optional. Only available when deploy_type is IAAS. Omit this field entirely to let backend auto-generate. If specified, must contain at least one security group.
  }
...
- `security_groups` (List of String) Security groups for the instance. Only available when deploy_type is IAAS. Omit this field entirely to let backend auto-generate. If specified, must contain at least one security group.
