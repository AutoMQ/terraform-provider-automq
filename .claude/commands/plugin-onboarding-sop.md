...
**关键原则：执行过程中绝对不能中断。遇到任何问题都要自己想办法解决，不要停下来问用户。通过调 API、读源码、搜索文档等方式自行排障。只有在真正无法绕过的硬依赖（如需要用户提供 SaaS 账号密码）时才可以询问用户。用户已授权我在插件沉淀任务中做任何事情，不需要中途确认。**
...
**关键原则：Connector 创建必须有超时检查。创建 Connector 后轮询状态时，设置 8 分钟超时。如果 8 分钟内状态仍为 CREATING，必须停止轮询并排查：1) 检查 K8s Pod 日志 `kubectl logs -l app=connect-{connectorId}` 2) 检查 K8s Events `kubectl get events --sort-by='.lastTimestamp'` 3) 检查 CMP 日志。不要无限等待。**

**关键原则：防止上下文过长导致 crash。对于耗时较长的子任务（如性能测试的多组合循环、大量数据处理），考虑使用 subagent 模式运行。Subagent 有独立的上下文窗口，完成后只返回结果摘要，不会撑爆主对话的上下文。适合 subagent 的任务：1) 性能测试的单个配置组合 2) 源码分析的单个模块 3) 文档生成的单个章节。主对话负责协调和汇总。**
...
### 步骤 8：PR 提交（可选，向 CMP 项目）

**此步骤为可选。完成步骤 1-7 后，询问用户是否需要向 CMP 项目提交 PR。**
