...
func cFlattenKafkaCluster(vo *client.ConnectorVO, prev *ConnectorKafkaClusterModel) *ConnectorKafkaClusterModel {
	m := &ConnectorKafkaClusterModel{KafkaInstanceID: types.StringNull()}
	var prevSec *SecurityProtocolConfigModel
	if prev != nil {
		m.KafkaInstanceID = prev.KafkaInstanceID
		prevSec = prev.Security
	}
	if vo.KafkaInstanceId != nil {
		m.KafkaInstanceID = types.StringValue(*vo.KafkaInstanceId)
	}
	m.Security = cFlattenSecurityProtocol(vo.SecurityProtocolConfig, prevSec)
	return m
}
...
func cFlattenSecurityProtocol(cfg *client.SecurityProtocolConfig, prev *SecurityProtocolConfigModel) *SecurityProtocolConfigModel {
	m := &SecurityProtocolConfigModel{
		SecurityProtocol: types.StringNull(), Username: types.StringNull(),
		Password: types.StringNull(), SaslMechanism: types.StringNull(),
		TruststoreCerts: types.StringNull(), KeyPassword: types.StringNull(),
		ClientCert: types.StringNull(), PrivateKey: types.StringNull(),
	}
	if prev != nil {
		m = prev
	}
